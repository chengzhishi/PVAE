...
class L0Pair(Module):
    ...
    def __init__(self, in_features, out_features, bias=True, weight_decay=1., droprate_init=0.5, temperature=1./20.,
        ...
        self.out_features = out_features
        self.prior_prec = weight_decay
        self.mean_weights = Parameter(torch.Tensor(in_features, out_features))
        self.var_weights = Parameter(torch.Tensor(in_features, out_features))
        #self.qz_loga = Parameter(torch.Tensor(in_features))
        self.qz_loga = Parameter(torch.Tensor(out_features))
        self.temperature = temperature
        self.droprate_init = droprate_init if droprate_init != 0. else 0.5
        self.lamba = lamba
        self.use_bias = False
        self.local_rep = local_rep
        self.mask = Variable(torch.zeros((1,out_features)), requires_grad=False)
        if bias:
            self.mean_bias = Parameter(torch.Tensor(out_features))
            self.var_bias = Parameter(torch.Tensor(out_features))
            self.use_bias = True
        self.floatTensor = torch.FloatTensor if not torch.cuda.is_available() else torch.cuda.FloatTensor
        self.reset_parameters()
    ...
    def reset_parameters(self):
        ...
        init.kaiming_normal_(self.var_weights, mode='fan_out')

        self.qz_loga.data.normal_(math.log(1 - self.droprate_init) - math.log(self.droprate_init), 1e-2)

        if self.use_bias:
            self.mean_bias.data.fill_(0)
            self.var_bias.data.fill_(0)
    ...
    def _reg_w(self):
        """Expected L0 norm under the stochastic gates, takes into account and re-weights also a potential L2 penalty"""
        logpw_col = torch.sum( (.5 * self.prior_prec * (self.mean_weights.pow(2)+self.var_weights.pow(2))) + self.lamba, 0)
        logpw = torch.sum((1 - self.cdf_qz(0)) * logpw_col)
        logpb = 0 if not self.use_bias else - torch.sum(.5 * self.prior_prec * (self.mean_bias.pow(2)+self.var_bias.pow(2)))
        return logpw + logpb
    ...
    def sample_z(self, batch_size, sample=True):
        ...
        if sample:
            ...
            pi = F.sigmoid(self.qz_loga).view(1, self.out_features).expand(batch_size, self.out_features)
            return F.hardtanh(pi * (limit_b - limit_a) + limit_a, min_val=0, max_val=1)
    ...
    def sample_mask(self):
        ...
        # mask = F.hardtanh(z, min_val=0, max_val=1)
        # return mask.view(self.in_features, 1) * self.weights
        z = self.quantile_concrete(F.hardtanh(0.8*self.get_eps(self.floatTensor(self.out_features))+0.1))
        mask = F.hardtanh(z, min_val=0, max_val=1)
        return mask.view(1, self.out_features)
    ...
    def forward(self, input):
        if self.local_rep or not self.training:
            ...
            #print("mask",mask)
            mean = input.mm(self.mean_weights) * mask
            var = input.mm(self.var_weights) * mask

        if self.use_bias:
            mean.add_(self.mean_bias)
            var.add_(self.var_bias)
