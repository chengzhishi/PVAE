...
if __name__ == '__main__':
    ...
    parser.add_argument('--beta-tc', type = float, default = 4.0)
    parser.add_argument('--dip-vae-lam', type = float, default = 3.0)
    args = parser.parse_args()

    # gin_bindings[2] = "beta_tc_vae.beta = " + str(args.beta_tc)
    gin_bindings[2] = "dip_vae.lambda_od = " + str(args.dip_vae_lam)
    gin_bindings[3] = "dip_vae.lambda_d_factor = " + str(args.dip_vae_lam * 10)

    elapsed_time, gin_bindings = train_tf_main()

    result_path = './results/final_score_tf.txt'
    postprocess_json = os.path.join(experiment_output_path, "representation/results/aggregate/postprocess.json")
    ...
    with open(result_path, 'a+') as fw:
        ...
        with open('./tensorflow/model.gin', 'r') as fr:
            for line in fr.readlines():
                if re.split("\s+", line.strip())[0] == "vae_optimizer.learning_rate":
                    fw.write(line)
        with open(postprocess_json, 'r') as fr:
            for line in fr.readlines():
                fw.write(line)
        fw.write('\n')
